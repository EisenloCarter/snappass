...
public class SMS4 {
    ...
    private static byte[] Sbox={//S盒  因为一个字节表示的范围是-128-127，若该16进制超过该范围就不能用8位来表示了，0x44表示68在表示范围之内，是8位，而0xd6表示214，如果不在前面加byte，214用一个字节则表示不来，用byte进行限定它为字节类型。
    ...
    private int[] CK={//固定参数取值CK(i,j)=(4i+j)*7(mod256)
    ...
    {//加解密算法
        ...
        {//与输入明文的一个分组128位分为4个字相对，最后将4个字转换成一个字4个字节，共转换成16个字节
	...
	{//密钥扩展算法
		...
		for(int i=0;i<4;i++)
		{//将128位密钥分为4个32位的字x[i]
			tmp[0]=Key[0+4*i]&0xFF;//按位与
			tmp[1]=Key[1+4*i]&0xff;
			tmp[2]=Key[2+4*i]&0xff;
			tmp[3]=Key[3+4*i]&0xff;
			x[i]=tmp[0]<<24|tmp[1]<<16|tmp[2]<<8|tmp[3];//四个字节组成一个字，通过左移操作再进行按位或进行操作
		...
		{//生成轮密钥rk
			...
			mid=ByteSub(mid);           //T变换，包括先进行S盒置换，再进行线性变换L
