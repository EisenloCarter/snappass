...
public class UploadUtil {
	...
	public void uploadFile(String filePath, String fileKey, String RequestURL, Map<String, String> param) {
		...
		try {
			...
			//String prefix=tmpName.substring(tmpName.lastIndexOf(".")+1);
			File _file = new File(getSDCardPath()+tmpName);
			runEncryptFile(file,_file);
	...
	private void toUploadFile(File file, String fileKey, String RequestURL, Map<String, String> param) {
		...
		try {
            ...
            sb.append("Content-Type:application/pdf" + LINE_END);
            sb.append("Content-Type:application/msword" + LINE_END);
			sb.append(LINE_END);
			params = sb.toString();
			sb = null;
...
//			dos.write(tempOutputStream.toByteArray());
			...
			if (res == 200) {
				...
				//保存文件名至文件列表
				MainActivity.writeFileList(file.getName());
				return;
			} else {
	...
	private void runEncryptFile(File file,File _file) {
		...
		SMS4 sm4 = new SMS4();
		try {
			SecurityUtil.encryptFile(file,_file);
		} catch (Exception e) {
